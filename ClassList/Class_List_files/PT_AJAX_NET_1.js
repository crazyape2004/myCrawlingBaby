 ...
 defaultError:function()
 {
  ...
  processPIAPostParams: function() {
        ...
        if ((typeof bCleanHtml != "undefined") && bCleanHtml) { 
            ...
            for (var i = 0; i < hiddenObj.childNodes.length; i++) {
                var eObj = hiddenObj.childNodes[i];
                if (typeof eObj.id == 'undefined') continue;
                var eId = eObj.id;
           ...
           if (bDefer) chgFldArr = deferFldArr;

        } else {
             
            for (var i = 0; i < this.form.elements.length; i++) {
              var eObj = this.form.elements[i];
              var eId = eObj.id;              
        ...
        if (sTmpModal != "") this.paramsModal = sTmpModal;
        if (sTmpModal2 != "") this.paramsModal2 = sTmpModal2;
     ...
     if (scriptList)  // check redirect URL.  if it is redirect script, run it.
        ...
        for (var i=0; i < scriptList.length; i++)
            ...
            if (sTmp.indexOf("document.location=") == 0 || sTmp.indexOf("document.location =") == 0) {
                   var pos = 19;
                   var sOrigURL = scriptList[i].firstChild.data.substring(pos, (scriptList[i].firstChild.data).length - 2);
                    var sURL = preProcessUrl(sOrigURL, window, top, this.form);
                    scriptList[i].firstChild.data = scriptList[i].firstChild.data.substring(0, pos) + sURL + scriptList[i].firstChild.data.substring(sOrigURL.length, scriptList[i].firstChild.data.length);
